...
const ChangeShippingAddressDrawer = ({ data, is_ultron = true, is_store_front = false }: any) => {
	const {
		drawer,
		buyer_data,
		attribute_data,
	...
	const [selected_address_id, set_selected_address_id] = useState(attribute_data[SPECIAL_DOCUMENT_ATTRIBUTE.shipping_address]?.id);
	...
	const shipping_addresses: any = _.filter(all_addresses, (address: any) => {
		const type_obj = _.find(address.attributes, { id: 'type' });
		return type_obj?.value === BUYER_ADDRESS_TYPE.shipping;
	});
	const shipping_permission = get_permission(address_section, 'add', 'shipping');
	const has_customer_edit_permission: boolean = get_permission(address_section, 'edit', 'shipping')?.is_edit;
	const default_shipping_address_id = address_section?.default_shipping_address;
	const add_permission = shipping_addresses?.length < (address_section?.shipping_max_entities || Infinity) && shipping_permission?.is_add;
	...
	const attribute_type = shipping_addresses?.length > 1 ? DOC_LEVEL_ATTRS.ADDRESSES : DOC_LEVEL_ATTRS.ADDRESS;

	const doc_level_address: DocumentEntity | undefined = useMemo(
		() => utils.get_processed_document_entity(attribute_data, SPECIAL_DOCUMENT_ATTRIBUTE.shipping_address, address_attrs),
		[attribute_data, address_attrs],
	);
	...
	const handle_confirm = (selected_card_id?: string) => {
		...
		const payload_data = is_doc_level_selected
			? attribute_data?.[SPECIAL_DOCUMENT_ATTRIBUTE?.shipping_address]
			: _.find(shipping_addresses, { id: _address_id });
		handle_update_document_entity(
			shipping_addresses,
			_address_id,
			is_store_front ? SPECIAL_DOCUMENT_ATTRIBUTE?.shipping_address : drawer_type,
			payload_data,
			!_address_id, // for clear action
			undefined,
			set_btn_loading,
		);
	};
	...
	const handle_add_address_drawer = () => {
		...
		set_show_address_sheet_detail({ is_open: true, index: -1, is_shipping_type: true });
	};
	...
	const shipping_address_button = ({ sx = {} }) => {
		return (
			<Button sx={sx} startIcon={<Icon iconName='IconPlus' />} variant='outlined' onClick={handle_add_address_drawer}>
				{t('OrderManagement.Drawers.ShippingAddress')}
			</Button>
		);
	};

	const handle_render_empty_state = () => {
		if (is_store_front || (drawer?.drawer_type === 'shipping_address' && drawer?.drawer_state)) {
			return (
				<Grid container className={classes.empty_address}>
					<Icon iconName='IconShip' sx={{ height: '32px', width: '32px', color: theme?.order_management?.empty_address?.color }} />
					<CustomText type='Body'>{t('OrderManagement.ChangeAddressDrawer.NoShippingAddress')}</CustomText>
				</Grid>
			);
		} else
			return (
				<Grid container justifyContent={'center'} flexDirection={'column'} gap={is_ultron ? 2 : 0} alignItems={'center'}>
	...
	const render_address_box = useCallback(
		(address: DocumentEntity | undefined, key: string | number) => {
			const id = address?.id;
				...
				<Box
					...
					<Grid container padding={'1.5rem 1rem 0rem 1rem'} key={`address${key}`}>
						...
						{should_render_edit_btn && is_ultron && (
							<EditActionIcon
								on_edit_click={() => handle_edit_address(address, show_sync_back_edit_checkbox, show_sync_back_edit_info, true)}
							/>
						)}
					</Grid>
	...
	const address_not_found = () => {
		const is_address_present = _.some(shipping_addresses, { id: selected_address_id });

		return (
			is_store_front &&
			_.size(shipping_addresses) > 0 &&
			!is_address_present &&
			selected_address_id !== default_shipping_address_id &&
			!doc_level_address?.id
		);
	};

	useEffect(() => {
		if (attribute_data[SPECIAL_DOCUMENT_ATTRIBUTE.shipping_address]?.id) {
			set_selected_address_id(attribute_data[SPECIAL_DOCUMENT_ATTRIBUTE.shipping_address]?.id);
		}
		if (address_not_found()) {
			handle_confirm(default_shipping_address_id);
			set_selected_address_id(default_shipping_address_id);
		}
	}, [shipping_addresses?.length]);

	useEffect(() => {
		if (is_store_front && doc_level_address) set_selected_address_id(doc_level_address?.id);
	}, [doc_level_address]);

	useEffect(() => {
		if (is_store_front && shipping_address_button?.length > 0 && _.isEmpty(selected_address_id)) {
			handle_confirm(default_shipping_address_id);
			set_selected_address_id(default_shipping_address_id);
		}
		...
		<React.Fragment>
			{is_store_front && (
				<Grid container justifyContent={'space-between'} mb={2}>
					<CustomText type={is_ultron ? 'H2' : 'H3'}>{t('OrderManagement.Drawers.ShippingAddress')}</CustomText>
					{!is_small_screen && should_render_add_btn && shipping_address_button({})}
				</Grid>
			)}

			{is_ultron ? (
				<Box overflow={'scroll'} className={classes.drawerContentContainer}>
					{_.isEmpty(shipping_addresses) && !buyer_section_loading && handle_render_empty_state()}
					{buyer_section_loading ? (
						...
						<>
							{doc_level_address && (
								<DocumentLevelAttributeRenderer
									...
									render_divider={!_.isEmpty(shipping_addresses)}
								/>
							)}
							{_.map(shipping_addresses, (address: DocumentEntity, index: number) => {
				...
				<React.Fragment>
					...
					{buyer_section_loading ? (
						...
						<Grid container columnGap={2} alignItems='stretch'>
							...
							{_.map(shipping_addresses, (address: DocumentEntity, key: number) => {
								return (
									<Grid container alignItems='stretch' xs={12} sm={5} md={12} lg={3.8} xl={3.8}>
			...
			{is_small_screen && shipping_address_button({ sx: { width: '100%' } })}
...
export default ChangeShippingAddressDrawer;
