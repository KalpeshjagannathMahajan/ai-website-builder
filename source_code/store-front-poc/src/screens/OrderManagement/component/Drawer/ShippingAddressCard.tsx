...
function ShippingAddressCard({ address, default_shipping_address_id, address_section, selected_address_id }: Props) {
	const id = address?.id;
	...
	const hangle_render_additional_fields = (additional_fields: any) => {
		return _.map(additional_fields, (value: any, key) => {
			...
			const attributes =
				_.find(address_section?.addresses, (item) => item?.id === default_shipping_address_id) ||
