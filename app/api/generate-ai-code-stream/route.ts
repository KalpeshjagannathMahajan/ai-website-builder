...
/**
 * Helper function to read source_code files for reference
 */
function readSourceCodeFile(relativePath: string): string | null {
  try {
    const workspacePath = process.cwd();
    const fullPath = join(workspacePath, 'source_code', 'store-front-poc', relativePath);
    return readFileSync(fullPath, 'utf-8');
  } catch (error) {
    console.warn(`[generate-ai-code-stream] Could not read source_code file: ${relativePath}`, error);
    return null;
  }
}

/**
 * Check if prompt mentions products, categories, or e-commerce features
 */
function shouldIncludeSourceCodeContext(prompt: string): boolean {
  const keywords = [
    'product', 'category', 'catalog', 'collection',
...
1. **Home Page** - ALWAYS REQUIRED: Main landing page (Home.jsx or App.jsx) with navigation buttons/links to:
   - Product Listing Page
   - Category Listing Page
2. **Header with Navigation** - Usually Header.jsx containing nav with links to Home, Products, Categories
3. **Hero Section** - The main landing area (Hero.jsx) on the home page
4. **Main Content Sections** - Features, Services, About, etc. on the home page
5. **Product Listing Page** - ALWAYS REQUIRED: A page/component that uses POST /entity/v2/product/search API to display products
   - MUST have clickable product cards that navigate to Product Details Page
6. **Category Listing Page** - ALWAYS REQUIRED: A page/component that uses GET /entity/v3/category API to display categories
   - MUST have clickable category cards that trigger product listing API with category filter
7. **Product Details Page** - ALWAYS REQUIRED: A page/component that uses POST /entity/v2/product/detail API
   - Receives product ID from product card click/navigation
   - Displays complete product information
8. **Navigation/Routing** - ALWAYS REQUIRED: Set up routing or state management so users can navigate:
   - From Home page → Product Listing Page
   - From Home page → Category Listing Page
   - From Product Listing Page → Product Details Page (via product card click)
   - From Category Listing Page → Filtered Product Listing (via category card click)
   - Between all pages via Header navigation
9. **Footer** - Contact info, links, copyright (Footer.jsx)
10. **App.jsx** - Main app component that handles routing and imports all components
11. **API Config File** - src/config/api-config.js with the required API configuration
...
- ALWAYS include a Home Page (Home.jsx) with navigation buttons to Products and Categories
- ALWAYS include a Navigation/Header component (Nav.jsx or Header.jsx) with links to Home, Products, Categories
- ALWAYS include Product Listing Page - MUST use POST /entity/v2/product/search API
- ALWAYS include Category Listing Page - MUST use GET /entity/v3/category API
- ALWAYS set up navigation/routing so users can navigate from Home → Products and Home → Categories
- ALWAYS create src/config/api-config.js with API credentials

REQUIRED COMPONENTS for website clones:
1. Home.jsx or HomePage.jsx - ALWAYS REQUIRED: Main landing page with navigation buttons/links to Product and Category pages
2. Nav.jsx or Header.jsx - Navigation bar with links to Home, Products, Categories (NEVER SKIP THIS!)
3. Hero.jsx - Main landing section (displayed on Home page)
4. Features/Services/Products sections - Based on the site content (displayed on Home page)
5. ProductListing.jsx or ProductListingPage.jsx - ALWAYS REQUIRED: Component that uses POST /entity/v2/product/search API
   - MUST have clickable product cards that navigate to product details page
6. CategoryListing.jsx or CategoryPage.jsx - ALWAYS REQUIRED: Component that uses GET /entity/v3/category API
   - MUST have clickable category cards that filter products by category
7. ProductDetails.jsx or ProductDetailsPage.jsx - ALWAYS REQUIRED: Component that uses POST /entity/v2/product/detail API
   - Receives product ID from navigation/URL
   - Displays full product information
8. Footer.jsx - Footer with links and info
9. App.jsx - Main component that handles routing/navigation between Home, Products, Categories, ProductDetails pages
10. src/config/api-config.js - ALWAYS REQUIRED: API configuration file with credentials and helper functions
...
MANDATORY: Home Page Navigation
- Home page MUST have prominent buttons or links to navigate to:
  - Product Listing Page (e.g., "View Products" button)
  - Category Listing Page (e.g., "Browse Categories" button)
- These navigation elements should be visible and easily accessible (in Hero section or main content)
- Use onClick handlers or href links to navigate between pages
- If using React Router, set up routes for /, /products, /categories
- If using state-based navigation, use useState to manage current page/view

MANDATORY: Product and Category Listing Pages
- Product Listing Page MUST use: POST /entity/v2/product/search API (with request body containing catalog_ids, page, page_size)
- Category Listing Page MUST use: GET /entity/v3/category?catalog_id={catalogId} API
- Both pages MUST use real API calls (not mock data)
- Both pages MUST import and use the API_CONFIG from src/config/api-config.js
- 🚨 CRITICAL: ALL API calls MUST use API_CONFIG values - NEVER hardcode:
  * API_CONFIG.baseUrl for base URL
  * getApiHeaders() for headers
  * API_CONFIG.catalogId for catalog ID
  * API_CONFIG.tenantId for tenant ID (if needed)
- Both pages MUST include loading states and error handling
- Both pages MUST be responsive and use Tailwind CSS
- Reference the source_code implementations for patterns, but adapt to Tailwind CSS

CRITICAL: Product API Response Handling - VIOLATION = FAILURE
YOU MUST ACTUALLY RENDER THE PRODUCT DATA - NOT JUST FETCH IT

1. **Response Structure**:
   - API returns: { data: { hits: {...}, nbHits: number, page: number } } OR { hits: {...}, nbHits: number }
   - The hits field can be an OBJECT with product IDs as keys OR an ARRAY
   - If hits is an object: convert using Object.values(hits) to get array
   - If hits is an array: use directly

2. **Extract Products Correctly**:
   \`\`\`javascript
   const productData = result.data || result;
   const productHits = productData.hits || {};
   let productsArray = Array.isArray(productHits) 
     ? productHits 
     : Object.values(productHits);
   \`\`\`

3. **MUST Render Actual Product Fields** (DO NOT use placeholders):
   - Product Name: product.name || product.meta?.name || 'Unnamed Product'
   - Product Image: product.media?.[0]?.url || product.product_images?.[0] || placeholder image
   - Product Price: 
     * product.pricing?.final_price (single price)
     * product.pricing?.final_range (price range string)
     * product.pricing?.min_price && product.pricing?.max_price (range)
   - Product ID: product.id || product.entity_id (for key prop)
   - SKU: product.sku_id (if available)
   - Stock Status: product.inventory?.stock (show "In Stock" or "Out of Stock")

4. **Product Card Structure** (MUST implement):
   - Image container with fallback for missing images
   - Product name (truncate if too long)
   - Price display (formatted with $)
   - Stock status indicator
   - Responsive grid layout (grid-cols-1 md:grid-cols-3 lg:grid-cols-4)

5. **State Handling**:
   - Loading state: Show "Loading products..." spinner or skeleton
   - Error state: Show error message with retry option
   - Empty state: Show "No products found" message
   - Success state: Display products in grid

6. **FORBIDDEN**:
   - ❌ DO NOT leave comments like "Render product details"
   - ❌ DO NOT use placeholder text like "Product Name Here"
   - ❌ DO NOT skip rendering product fields
   - ❌ DO NOT forget to handle image loading errors
   - ❌ DO NOT forget to handle missing/null product fields

7. **REQUIRED**:
   - ✅ MUST render actual product.name in <h3> or similar
   - ✅ MUST render actual product image in <img> tag
   - ✅ MUST render actual price with proper formatting
   - ✅ MUST use product.id or product.entity_id as React key
   - ✅ MUST handle all edge cases (missing fields, null values)
...
2. ALWAYS generate src/config/api-config.js - this is required for product/category pages
...
6. Generate src/config/api-config.js with API credentials
7. Generate Home.jsx (ALWAYS REQUIRED - with navigation buttons to Products and Categories)
8. Generate ProductListing.jsx (ALWAYS REQUIRED - using POST /entity/v2/product/search API)
   - MUST include clickable product cards with onClick handlers
   - MUST navigate to ProductDetails page on product card click
9. Generate CategoryListing.jsx (ALWAYS REQUIRED - using GET /entity/v3/category API)
   - MUST include clickable category cards with onClick handlers
   - MUST call product listing API with category filter on category card click
10. Generate ProductDetails.jsx (ALWAYS REQUIRED - using POST /entity/v2/product/detail API)
    - MUST receive product ID from URL params or props
    - MUST call product details API with product_id, buyer_tenant_id, catalog_ids
    - MUST display all product information
11. Generate Header.jsx with navigation links to Home, Products, Categories
12. Generate App.jsx (must handle routing/navigation between Home, Products, Categories, ProductDetails pages)
13. Then generate EVERY SINGLE component file you imported (Hero, Footer, etc.)
14. Do NOT stop until all imports are satisfied
15. VERIFY navigation works: Home → Products/Categories, Product cards → ProductDetails, Category cards → Filtered products
...
EXISTING COMPONENT IMPLEMENTATIONS - SOURCE_CODE FOLDER:
When building e-commerce websites or applications that need product listing, category listing, or similar features:
1. **REFERENCE EXISTING IMPLEMENTATIONS**: The codebase contains reference implementations in the source_code/store-front-poc folder
2. **PRODUCT LISTING**: Reference source_code/store-front-poc/src/screens/ProductListing/ for:
   - ProductListing.tsx - Main product listing component
   - components/ExploreAllProductsListing.tsx - Product grid/list display
   - components/CategoriesRail.tsx - Category navigation rail
   - components/CollectionsRail.tsx - Collection display
   - utils/api_requests/productListing.ts - API request functions
3. **CATEGORY LISTING**: Reference source_code/store-front-poc/src/screens/ProductListing/components/Category.tsx for category display patterns
4. **API INTEGRATION**: When implementing product/category features:
   - Use the API_CONFIG from config/api-config.ts for authentication and base URLs
   - Implement actual API calls using fetch or axios
   - Use the provided endpoints: getCategories, getCollections, searchProducts
   - Include proper error handling and loading states
   - DO NOT use hardcoded mock data - use real API calls
5. **COMPONENT PATTERNS**: Study the existing components to understand:
   - How products are displayed (grid/list views)
   - How categories are fetched and rendered
   - How API responses are handled
   - How loading states and errors are managed
6. **ADAPTATION**: Adapt the patterns from source_code to your generated code:
   - Use Tailwind CSS instead of Material-UI
   - Simplify complex Redux patterns if not needed
   - Keep the core API integration logic
   - Maintain responsive design patterns

CATEGORY-BASED PRODUCT FILTERING - CRITICAL WORKFLOW:
1. Category Listing API returns category objects with id fields
2. Extract Category IDs: Get the id field from each category object returned by the category listing API
3. Filter Products by Category: When user selects a category, pass the category ID(s) in the product listing API request:
   - Use filters.category (NOT filters.category_ids) as the key
   - Pass an array of category IDs: filters.category: ["category-id-1", "category-id-2"]
   - The catalog_ids parameter is still REQUIRED (use API_CONFIG.catalogId)
4. Example Request Body for Category-Filtered Products:
   {
     catalog_ids: [API_CONFIG.catalogId],  // REQUIRED - always include
     filters: {
       category: ["category-id-from-category-listing"]  // Array of selected category IDs
     },
     page_number: 1,
     page_size: 20
   }
5. Implementation Pattern:
   - Fetch categories first using category listing API
   - Display categories to user (with click handlers)
   - When category is clicked, extract the category id
   - Call product listing API with filters.category: [categoryId] (replace categoryId with actual variable)
   - Display filtered products
6. Multiple Category Selection: To allow filtering by multiple categories, pass array of IDs:
   - filters.category: ["cat-id-1", "cat-id-2"] - shows products in ANY of these categories
7. All Products: If no category is selected, omit the filters.category parameter to show all products
8. See config/api-connection-docs.md for complete API connection documentation

INTERACTIVE FEATURES - CRITICAL REQUIREMENTS:
1. Clickable Category Cards:
   - Each category card in category listing page MUST be clickable
   - On category card click: Extract category.id and call product listing API with filters.category: [categoryId] (replace categoryId with actual variable)
   - Display filtered products when category is clicked
   - Use onClick handler on category cards: onClick={() => handleCategoryClick(category.id)}

2. Clickable Product Cards:
   - Each product card in product listing page MUST be clickable
   - On product card click: Extract product.id or product.entity_id and navigate to product details page
   - Use React Router: navigate('/product/' + productId) OR use template literal with productId variable OR state-based navigation
   - Use onClick handler on product cards: onClick={() => handleProductClick(product)}

3. Product Details Page:
   - MUST create ProductDetails.jsx component
   - MUST call POST /entity/v2/product/detail API with:
     * product_id: from URL params or props
     * buyer_tenant_id: API_CONFIG.buyerId
     * catalog_ids: [API_CONFIG.catalogId]
   - MUST display product information: images, name, price, stock, attributes, description
   - Reference source_code/store-front-poc/src/utils/api_requests/productDetails.ts for API implementation
   - Reference source_code/store-front-poc/src/screens/ProductDetailsPage for rendering patterns

4. Navigation Setup:
   - Set up React Router with routes: /products, /product/:productId (use :productId as route parameter), /categories
   - OR use state-based navigation with view switching
   - Ensure product cards navigate to product details page
   - Ensure category cards trigger product listing with filter

API CONFIGURATION FOR WEBSITE PREVIEW:
When generating websites that need to display products or categories:
1. **CREATE API CONFIG FILE**: You MUST create src/config/api-config.js in the generated website with these exact values:
   \`\`\`javascript
   ...
   \`\`\`
2. **API CALLS - CRITICAL REQUIREMENTS**:
   🚨 ALL API CALLS MUST USE API_CONFIG - NEVER HARDCODE VALUES 🚨
   - Base URL: ALWAYS use API_CONFIG.baseUrl (NEVER hardcode 'https://ultronapi-worker-g-p.sourcerer.tech')
   - Authentication: ALWAYS use getApiHeaders() function (NEVER hardcode token or headers)
   - Catalog ID: ALWAYS use API_CONFIG.catalogId (NEVER hardcode the catalog ID)
   - Tenant ID: ALWAYS use API_CONFIG.tenantId (NEVER hardcode the tenant ID)
   - Buyer ID: ALWAYS use API_CONFIG.buyerId (NEVER hardcode the buyer ID)
   - Headers: ALWAYS use getApiHeaders() function (NEVER manually create headers object)
   
   ❌ FORBIDDEN - DO NOT DO THIS:
   - fetch('https://ultronapi-worker-g-p.sourcerer.tech/entity/v2/product/search', ...) // WRONG - hardcoded URL
   - headers: { 'Authorization': 'eyJhbGci...' } // WRONG - hardcoded token
   - catalog_ids: ['1da021a7-7e91-4063-9e1f-b387c87d1ad4'] // WRONG - hardcoded catalog ID
   
   ✅ REQUIRED - DO THIS:
   - fetch(\`\${API_CONFIG.baseUrl}/entity/v2/product/search\`, ...) // CORRECT
   - headers: getApiHeaders() // CORRECT
   - catalog_ids: [API_CONFIG.catalogId] // CORRECT
3. **ENDPOINTS TO USE**:
   - **Product Listing Page MUST use**: POST /entity/v2/product/search
     - Method: POST
     - Request body: { catalog_ids: [catalogId], page: 1, page_size: 20 }
     - Returns: { hits: [...products], nbHits: number, page: number }
   - **Category Listing Page MUST use**: GET /entity/v3/category?catalog_id={catalogId}
     - Method: GET
     - Query param: catalog_id
     - Returns: Array of category objects
   - GET /entity/v3/collection/search?catalog_id={catalogId} - Optional: For collections
4. **EXAMPLE API CALLS IN COMPONENTS**:
   
   **Product Listing Page Example (MUST USE API_CONFIG - NO HARDCODING):**
   \`\`\`javascript
   // CRITICAL: MUST import API_CONFIG and getApiHeaders
     ...
     useEffect(() => {
       const fetchProducts = async () => {
         try {
           ...
           // 🚨 CRITICAL: MUST use API_CONFIG.baseUrl - NEVER hardcode the URL
           // ❌ WRONG: fetch('https://ultronapi-worker-g-p.sourcerer.tech/entity/v2/product/search', ...)
           // ✅ CORRECT: fetch(\`\${API_CONFIG.baseUrl}/entity/v2/product/search\`, ...)
           const response = await fetch(
             \`\${API_CONFIG.baseUrl}/entity/v2/product/search\`,
             {
               method: 'POST',
               // 🚨 CRITICAL: MUST use getApiHeaders() - NEVER hardcode headers or tokens
               // ❌ WRONG: headers: { 'Authorization': 'eyJhbGci...', 'Content-Type': 'application/json' }
               // ✅ CORRECT: headers: getApiHeaders()
               headers: getApiHeaders(),
               body: JSON.stringify({
                 // 🚨 CRITICAL: MUST use API_CONFIG.catalogId - NEVER hardcode the catalog ID
                 // ❌ WRONG: catalog_ids: ['1da021a7-7e91-4063-9e1f-b387c87d1ad4']
                 // ✅ CORRECT: catalog_ids: [API_CONFIG.catalogId]
                 catalog_ids: [API_CONFIG.catalogId],
                 page: 1,
                 page_size: 20
               })
           ...
           // IMPORTANT: Extract products from response
           // Response structure: { data: { hits: {...}, nbHits: number, page: number } }
           // OR direct: { hits: {...}, nbHits: number, page: number }
           // Note: hits can be an object with product IDs as keys OR an array
           const productData = result.data || result;
           const productHits = productData.hits || {};
           
           // Convert hits to array - handle both object and array formats
           let productsArray = [];
           if (Array.isArray(productHits)) {
             productsArray = productHits;
           } else if (typeof productHits === 'object' && productHits !== null) {
             // If hits is an object with product IDs as keys, convert to array
             productsArray = Object.values(productHits);
           }
       ...
       <div className="container mx-auto p-4">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
           {products.map((product, index) => {
             // Extract product fields - handle different response structures
             const productId = product.id || product.entity_id || 'product-' + index;
             const productName = product.name || product.meta?.name || 'Unnamed Product';
             const productImage = product.media?.[0]?.url || product.product_images?.[0] || '';
             
             // Extract pricing - handle different pricing structures
   ...
   **Category Listing Page Example (MUST USE API_CONFIG - NO HARDCODING):**
   \`\`\`javascript
   // CRITICAL: MUST import API_CONFIG and getApiHeaders
   import { API_CONFIG, getApiHeaders } from '../config/api-config';
   import { useState, useEffect } from 'react';
   ...
   function CategoryListing() {
     ...
     useEffect(() => {
       const fetchCategories = async () => {
         try {
           // 🚨 CRITICAL: MUST use API_CONFIG.baseUrl and API_CONFIG.catalogId
           // ❌ WRONG: fetch('https://ultronapi-worker-g-p.sourcerer.tech/entity/v3/category?catalog_id=1da021a7-7e91-4063-9e1f-b387c87d1ad4', ...)
           // ✅ CORRECT: fetch(\`\${API_CONFIG.baseUrl}/entity/v3/category?catalog_id=\${API_CONFIG.catalogId}\`, ...)
           const response = await fetch(
             \`\${API_CONFIG.baseUrl}/entity/v3/category?catalog_id=\${API_CONFIG.catalogId}\`,
             {
               method: 'GET',
               // 🚨 CRITICAL: MUST use getApiHeaders() - NEVER hardcode headers
               headers: getApiHeaders()
             }
           );
   ...
   \`\`\`
5. **CRITICAL REQUIREMENTS - API CONFIG USAGE**:
   🚨 MANDATORY: ALL API CALLS MUST USE API_CONFIG - NO EXCEPTIONS 🚨
   
   - The website MUST use real API calls, NOT mock data
   - ALWAYS import API_CONFIG and getApiHeaders from '../config/api-config' (or correct relative path)
   - ALWAYS use API_CONFIG.baseUrl for all API URLs (NEVER hardcode the base URL)
   - ALWAYS use getApiHeaders() for all request headers (NEVER hardcode headers or tokens)
   - ALWAYS use API_CONFIG.catalogId in request bodies/query params (NEVER hardcode catalog ID)
   - ALWAYS use API_CONFIG.tenantId if needed (NEVER hardcode tenant ID)
   - ALWAYS use API_CONFIG.buyerId if needed (NEVER hardcode buyer ID)
   - Create the api-config.js file in src/config/ directory with the exact structure shown above
   - Import and use it in EVERY component that makes API calls
   
   ❌ ABSOLUTELY FORBIDDEN:
   - Hardcoding base URL: 'https://ultronapi-worker-g-p.sourcerer.tech'
   - Hardcoding authentication token: 'eyJhbGci...'
   - Hardcoding catalog ID: '1da021a7-7e91-4063-9e1f-b387c87d1ad4'
   - Hardcoding tenant ID: 'f0521546-bb18-497e-aea1-61cdab5399e6'
   - Creating headers object manually instead of using getApiHeaders()
   - Using environment variables instead of API_CONFIG
   ...
   const response = await fetch(
     ...
     {
       method: 'POST',
       headers: getApiHeaders(), // MUST use this function
       body: JSON.stringify({
         catalog_ids: [API_CONFIG.catalogId] // MUST use from config
       })
     }
   );
   \`\`\``;
...
`;
        ...
        if (context) {
          ...
          if (isEdit) {
            ...
            contextParts.push('3. **HOME PAGE** - ALWAYS REQUIRED: Create Home.jsx with navigation buttons/links to Products and Categories');
            contextParts.push('4. **COMPLETE COMPONENTS** - Header, Hero, Features, Footer on Home page');
            contextParts.push('5. **PRODUCT LISTING PAGE** - ALWAYS REQUIRED: Create ProductListing.jsx using POST /entity/v2/product/search API');
            contextParts.push('6. **CATEGORY LISTING PAGE** - ALWAYS REQUIRED: Create CategoryListing.jsx using GET /entity/v3/category API');
            contextParts.push('7. **NAVIGATION SETUP** - ALWAYS REQUIRED: Set up routing/state so users can navigate from Home → Products and Home → Categories');
            contextParts.push('8. **API CONFIG FILE** - ALWAYS REQUIRED: Create src/config/api-config.js with API credentials');
            contextParts.push('9. **VISUAL POLISH** - Shadows, hover states, transitions');
            contextParts.push('10. **STANDARD CLASSES** - bg-white, text-gray-900, bg-blue-500, NOT bg-background');
            contextParts.push('\n🚨 MANDATORY REQUIREMENTS:');
            contextParts.push('- Home page MUST have prominent buttons/links: "View Products" and "Browse Categories"');
            contextParts.push('- Product listing page MUST use POST /entity/v2/product/search API (not GET)');
            contextParts.push('- Category listing page MUST use GET /entity/v3/category?catalog_id={catalogId} API');
            contextParts.push('- 🚨 CRITICAL: ALL API calls MUST use API_CONFIG - NEVER hardcode URLs, tokens, or IDs');
            contextParts.push('- ALWAYS use API_CONFIG.baseUrl for base URL (never hardcode)');
            contextParts.push('- ALWAYS use getApiHeaders() for headers (never hardcode headers or tokens)');
            contextParts.push('- ALWAYS use API_CONFIG.catalogId in requests (never hardcode catalog ID)');
            contextParts.push('- Product API response: Extract products from response.data.hits OR response.hits');
            contextParts.push('- Product API response structure: { data: { hits: [...], nbHits: number } } OR { hits: [...], nbHits: number }');
            contextParts.push('- CRITICAL: MUST render actual product data (name, image, price, stock) - NO placeholder comments');
            contextParts.push('- Handle both array and object formats for hits - convert object to array if needed');
            contextParts.push('- Show empty state when products.length === 0');
            contextParts.push('- Show error state when API call fails');
            contextParts.push('- Display products in grid with product cards showing: image, name, price, stock status');
            contextParts.push('- Both pages MUST include loading states and error handling');
            contextParts.push('- Navigation MUST work: Home page buttons → Product/Category pages, Header links → all pages');
            contextParts.push('- Use React Router for multi-page navigation OR useState for single-page with view switching');
            contextParts.push('- API config file MUST be created with all required credentials');
            contextParts.push('- Import API_CONFIG and getApiHeaders in EVERY component that makes API calls');
          ...
          // Add source_code reference implementations if relevant
          if (shouldIncludeSourceCodeContext(prompt)) {
            contextParts.push('\n📚 REFERENCE IMPLEMENTATIONS FROM SOURCE_CODE:');
            contextParts.push('The following files from source_code/store-front-poc contain reference implementations:');
            contextParts.push('Study these patterns and adapt them to your generated code using Tailwind CSS.');
            
            // Read key reference files
            const referenceFiles = [
              { path: 'src/utils/api_requests/productListing.ts', name: 'Product Listing API Functions' },
              { path: 'src/screens/ProductListing/ProductListing.tsx', name: 'Main Product Listing Component' },
              { path: 'src/screens/ProductListing/components/ExploreAllProductsListing.tsx', name: 'Product Grid/List Display' },
              { path: 'src/screens/ProductListing/components/CategoriesRail.tsx', name: 'Category Navigation Component' },
              { path: 'src/screens/ProductListing/components/Category.tsx', name: 'Category Display Component' },
            ];
            
            for (const ref of referenceFiles) {
              const content = readSourceCodeFile(ref.path);
              if (content) {
                // Include a preview (first 2000 chars) to avoid token bloat
                const preview = content.length > 2000 ? content.substring(0, 2000) + '\n... (truncated)' : content;
                contextParts.push(`\n<reference_file path="${ref.path}" name="${ref.name}">`);
                contextParts.push(preview);
                contextParts.push('</reference_file>');
              }
            }
            
            contextParts.push('\nIMPORTANT: When implementing similar features:');
            contextParts.push('1. Adapt the API call patterns from productListing.ts');
            contextParts.push('2. Use the component structure as a guide, but convert to Tailwind CSS');
            contextParts.push('3. Implement real API calls using the API_CONFIG from config/api-config.ts');
            contextParts.push('4. Include proper loading states, error handling, and responsive design');
          }
